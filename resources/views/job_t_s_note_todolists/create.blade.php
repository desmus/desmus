...
@section('scripts')
  ...
  <script>
    ...
    $('#job_t_s_note_todolist_create').on('submit', function() {
      ...
      var job_t_s_note_todolist_status = document.getElementById("status").value;
      var job_t_s_note_todolist_datetime = document.getElementById("datetime").value;
      
      var date_format = /^(\d{4})-(\d{1,2})-(\d{1,2})$/;
      
      if(job_t_s_note_todolist_name.length >= 100)
      {
        alert("Invalid character number for the task name.");
        return false;
      }
      
      if(job_t_s_note_todolist_description.length >= 1000)
      {
        alert("Invalid character number for the description.");
        return false;
      }
  
      if(job_t_s_note_todolist_datetime != '')
      {
        if(date_formats = job_t_s_note_todolist_datetime.match(date_format))
        {
        ...
        {
          alert("Invalid date format: " + job_t_s_note_todolist_datetime);
          return false;
        }
      }
      
      if(job_t_s_note_todolist_name == '' || job_t_s_note_todolist_description == '' || job_t_s_note_todolist_status == '' || job_t_s_note_todolist_datetime == '')
      {
...
@section('content')
  ...
  <section class="content-header">
    ...
    <h1> Job Topic Section Note Todolist </h1>
      ...
      <div class="box-body">
        ...
        <div class="row">
          ...
          {!! Form::open(['route' => 'jobTSNoteTodolists.store', 'id' => 'job_t_s_note_todolist_create']) !!}
            ...
            @include('job_t_s_note_todolists.fields')
