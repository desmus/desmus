...
use App\Http\Requests\CreateCollegeTSGalerieRequest;
use App\Http\Requests\UpdateCollegeTSGalerieRequest;
use App\Repositories\CollegeTSGalerieRepository;
...
class CollegeTSGalerieController extends AppBaseController
{
    private $collegeTSGalerieRepository;

    public function __construct(CollegeTSGalerieRepository $collegeTSGalerieRepo)
    {
        $this->collegeTSGalerieRepository = $collegeTSGalerieRepo;
    }
    ...
    public function index(Request $request)
    {
        ...
        {
            $this->collegeTSGalerieRepository->pushCriteria(new RequestCriteria($request));
            $collegeTSGaleries = $this->collegeTSGalerieRepository->all();
    
            return view('college_t_s_galeries.index')
    ...
    public function create($id)
    {
        if(Auth::user() != null)
        {
            ...
            return view('college_t_s_galeries.create')
                ->with('id', $id)
    ...
    public function store(CreateCollegeTSGalerieRequest $request)
    {
        if(Auth::user() != null)
        {
            $now = Carbon::now();
            ...
            $input = $request->all();
            $collegeTSGalerie = $this->collegeTSGalerieRepository->create($input);
            
            DB::table('college_t_s_galery_creates')->insert(['datetime' => $now, 'user_id' => $user_id, 'college_t_s_galery_id' => $collegeTSGalerie -> id]);
            DB::table('recent_activities')->insert(['name' => $collegeTSGalerie -> name, 'status' => 'active', 'type' => 'c_t_s_g_c', 'user_id' => $user_id, 'entity_id' => $collegeTSGalerie -> id, 'created_at' => $now]);
    
            Flash::success('College T S Galerie saved successfully.');
            return redirect(route('collegeTopicSections.show', [$collegeTSGalerie -> college_topic_section_id]));
        }
    ...
    public function show($id, Request $request)
    {
        if(Auth::user() != null)
        {
            $college_image_p = $request -> college_image_p;
            
            $now = Carbon::now();
            $user_id = Auth::user()->id;
            $collegeTSGalerie = $this->collegeTSGalerieRepository->findWithoutFail($id);
            
            if(empty($collegeTSGalerie))
            {
                Flash::error('College T S Galerie not found');
                return redirect(route('collegeTSGaleries.index'));
            }
            ...
            {
                DB::table('college_t_s_galery_views')->insert(['datetime' => $now, 'user_id' => $user_id, 'college_t_s_galery_id' => $id]);
                DB::table('college_t_s_galeries')->where('id', $id)->update(['views_quantity' => DB::raw('views_quantity + 1')]);
                
                $collegeTSGalerie = $this->collegeTSGalerieRepository->findWithoutFail($id);
                $collegeTSGaleryImages = DB::table('college_t_s_galery_images')->where('college_t_s_g_id' , '=', $id)->orderBy('id', 'desc')->paginate(100, ['*'], 'college_image_p');
                $collegeTopicSectionGaleryViews = DB::table('users')->join('college_t_s_galery_views', 'users.id', '=', 'college_t_s_galery_views.user_id')->where('college_t_s_galery_id', $id)->orderBy('datetime', 'desc')->limit(50)->get();
                $collegeTopicSectionGaleryUpdates = DB::table('users')->join('college_t_s_galery_updates', 'users.id', '=', 'college_t_s_galery_updates.user_id')->where('college_t_s_galery_id', $id)->orderBy('datetime', 'desc')->limit(50)->get();
                $collegeTSGaleryTodolist = DB::table('colleges')->join('college_topics', 'colleges.id', '=', 'college_topics.college_id')->join('college_topic_sections', 'college_topics.id', '=', 'college_topic_sections.college_topic_id')->join('college_t_s_galeries', 'college_topic_sections.id', '=', 'college_t_s_galeries.college_topic_section_id')->join('college_t_s_galery_todolists', 'college_t_s_galeries.id', '=', 'college_t_s_galery_todolists.c_t_s_g_id')->where('college_t_s_galery_todolists.c_t_s_g_id', '=', $collegeTSGalerie -> id)->where(function ($query) {$query->where('college_t_s_galery_todolists.deleted_at', '=', null);})->where(function ($query) {$query->where('college_t_s_galery_todolists.status', '=', 'active');})->orderBy('college_t_s_galery_todolists.datetime', 'desc')->limit(50)->get();
                $collegeTSGaleryTodolistCompleted = DB::table('colleges')->join('college_topics', 'colleges.id', '=', 'college_topics.college_id')->join('college_topic_sections', 'college_topics.id', '=', 'college_topic_sections.college_topic_id')->join('college_t_s_galeries', 'college_topic_sections.id', '=', 'college_t_s_galeries.college_topic_section_id')->join('college_t_s_galery_todolists', 'college_t_s_galeries.id', '=', 'college_t_s_galery_todolists.c_t_s_g_id')->where('college_t_s_galery_todolists.c_t_s_g_id', '=', $collegeTSGalerie -> id)->where(function ($query) {$query->where('college_t_s_galery_todolists.status', '=', 'finalized');})->where(function ($query) {$query->where('college_t_s_galery_todolists.deleted_at', '=', null);})->orderBy('college_t_s_galery_todolists.datetime', 'desc')->limit(50)->get();

                $collegeTSGImagesList = DB::table('college_t_s_galery_images')->where('college_t_s_g_id' , '=', $id)->orderBy('id', 'desc')->limit(10)->get();
                $collegeTSGaleryTodolistsList = DB::table('colleges')->join('college_topics', 'colleges.id', '=', 'college_topics.college_id')->join('college_topic_sections', 'college_topics.id', '=', 'college_topic_sections.college_topic_id')->join('college_t_s_galeries', 'college_topic_sections.id', '=', 'college_t_s_galeries.college_topic_section_id')->join('college_t_s_galery_todolists', 'college_t_s_galeries.id', '=', 'college_t_s_galery_todolists.c_t_s_g_id')->where('college_t_s_galery_todolists.c_t_s_g_id', '=', $collegeTSGalerie -> id)->where(function ($query) {$query->where('college_t_s_galery_todolists.deleted_at', '=', null);})->where(function ($query) {$query->where('college_t_s_galery_todolists.status', '=', 'active');})->orderBy('college_t_s_galery_todolists.datetime', 'desc')->limit(5)->get();
                $collegeTSGaleryTodolistsCompletedList = DB::table('colleges')->join('college_topics', 'colleges.id', '=', 'college_topics.college_id')->join('college_topic_sections', 'college_topics.id', '=', 'college_topic_sections.college_topic_id')->join('college_t_s_galeries', 'college_topic_sections.id', '=', 'college_t_s_galeries.college_topic_section_id')->join('college_t_s_galery_todolists', 'college_t_s_galeries.id', '=', 'college_t_s_galery_todolists.c_t_s_g_id')->where('college_t_s_galery_todolists.c_t_s_g_id', '=', $collegeTSGalerie -> id)->where(function ($query) {$query->where('college_t_s_galery_todolists.status', '=', 'finalized');})->where(function ($query) {$query->where('college_t_s_galery_todolists.deleted_at', '=', null);})->orderBy('college_t_s_galery_todolists.datetime', 'desc')->limit(5)->get();
                ...
                return view('college_t_s_galeries.show')
                    ->with('collegeTSGalerie', $collegeTSGalerie)
                    ->with('collegeTSGaleryImages', $collegeTSGaleryImages)
                    ->with('collegeTSGaleryViews', $collegeTopicSectionGaleryViews)
                    ->with('collegeTSGaleryUpdates', $collegeTopicSectionGaleryUpdates)
                    ->with('collegeTSGaleryTodolist', $collegeTSGaleryTodolist)
                    ->with('collegeTSGaleryTodolistCompleted', $collegeTSGaleryTodolistCompleted)
                    ->with('college_image_p', $college_image_p)
                    ->with('user_id', $user_id)
                    ->with('user', $user)
                    ->with('now', $now)
                    ->with('id', $id)
                    ->with('collegeTSGImagesList', $collegeTSGImagesList)
                    ->with('collegeTSGaleryTodolistsList', $collegeTSGaleryTodolistsList)
                    ->with('collegeTSGaleryTodolistsCompletedList', $collegeTSGaleryTodolistsCompletedList)
    ...
    public function edit($id)
    {
        if(Auth::user() != null)
        {
            ...
            foreach($userCollegeTSGaleries as $userCollegeTSGalerie)
            {
                if($userCollegeTSGalerie -> user_id == $user_id && $userCollegeTSGalerie -> permissions == 'advanced')
                {
                    $isShared = true;
                    break;
                }
            ...
            if($user_id == $user[0] -> id || $isShared)
            {
                ...
                return view('college_t_s_galeries.edit')
                    ->with('collegeTSGalerie', $collegeTSGalerie)
                    ->with('collegeTSGImagesList', $collegeTSGImagesList)
                    ->with('collegeTSGaleryTodolistsList', $collegeTSGaleryTodolistsList)
                    ->with('collegeTSGaleryTodolistsCompletedList', $collegeTSGaleryTodolistsCompletedList)
    ...
    public function update($id, UpdateCollegeTSGalerieRequest $request)
    {
        if(Auth::user() != null)
        {
            $now = Carbon::now();
            ...
            {
                $newCollegeTSGalerie = $this->collegeTSGalerieRepository->update($request->all(), $id);
        
                DB::table('college_t_s_galeries')->where('id', $id)->update(['updates_quantity' => DB::raw('updates_quantity + 1')]);
                DB::table('college_t_s_galery_updates')->insert(['actual_name' => $newCollegeTSGalerie -> name, 'past_name' => $collegeTSGalerie -> name, 'datetime' => $now, 'college_t_s_galery_id' => $id, 'user_id' => $user_id]);
                DB::table('recent_activities')->insert(['name' => $collegeTSGalerie -> name, 'status' => 'active', 'type' => 'c_t_s_g_u', 'user_id' => $user_id, 'entity_id' => $collegeTSGalerie -> id, 'created_at' => $now]);
            
                Flash::success('College T S Galerie updated successfully.');
                return redirect(route('collegeTSGaleries.show', [$id]));
            }
    ...
    public function destroy($id)
    {
        if(Auth::user() != null)
        {
            ...
            {
                $collegeTSGaleryImages = DB::table('college_t_s_galery_images')->where('college_t_s_g_id', '=', $collegeTSGalerie -> id)->where(function ($query) {$query->where('deleted_at', '=', null);})->get();
    
                DB::table('user_college_t_s_galeries')->where('college_t_s_galery_id', $collegeTSGalerie -> id)->update(['deleted_at' => $now]);
                
                $userCollegeTSGalery = DB::table('user_college_t_s_galeries')->where('college_t_s_galery_id', '=', $collegeTSGalerie -> id)->orderBy('datetime', 'desc')->limit(1)->get();
                ...
                $this->collegeTSGalerieRepository->delete($id);
                $collegeTSGaleryImages = DB::table('college_t_s_galery_images')->where('college_t_s_g_id', '=', $collegeTSGalerie -> id)->where(function ($query) {$query->where('deleted_at', '=', null);})->get();
                ...
                DB::table('college_t_s_galery_deletes')->insert(['datetime' => $now, 'user_id' => $user_id, 'college_t_s_galery_id' => $collegeTSGalerie -> id]);
                DB::table('recent_activities')->insert(['name' => $collegeTSGalerie -> name, 'status' => 'active', 'type' => 'c_t_s_g_d', 'user_id' => $user_id, 'entity_id' => $collegeTSGalerie -> id, 'created_at' => $now]);
            
                Flash::success('College T S Galerie deleted successfully.');
                return redirect(route('collegeTopicSections.show', [$collegeTSGalerie -> college_topic_section_id]));
            }
