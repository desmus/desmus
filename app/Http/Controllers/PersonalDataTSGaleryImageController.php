    ...
    public function show($id)
    {
        if(Auth::user() != null)
        {
            ...
            if($user_id == $user[0] -> id || $isShared)
            {
                ...
                $personalDataTSGaleryImage = $this->personalDataTSGaleryImageRepository->findWithoutFail($id);
                $personalDataTopicSectionGaleryImageViews = DB::table('users')->join('personal_data_t_s_galery_image_views', 'users.id', '=', 'personal_data_t_s_galery_image_views.user_id')->where('p_d_t_s_g_i_id', $id)->orderBy('datetime', 'desc')->limit(100)->get();
                $personalDataTopicSectionGaleryImageUpdates = DB::table('users')->join('personal_data_t_s_galery_image_updates', 'users.id', '=', 'personal_data_t_s_galery_image_updates.user_id')->where('p_d_t_s_g_i_id', $id)->orderBy('datetime', 'desc')->limit(100)->get();
